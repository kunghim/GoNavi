...
func TestMySQLApplyChangesQuotesBacktickIdentifiers(t *testing.T) {
	for _, driverType := range []string{"mysql", "goldendb"} {
		for _, testCase := range []struct {
			...
			t.Run(driverType+"/"+testCase.name, func(t *testing.T) {
				state := &writeOutcomeTransactionState{}
				databaseConnection := openWriteOutcomeTransactionDB(t, state)
				databaseInstance, err := NewDatabase(driverType)
				if err != nil {
					t.Fatalf("NewDatabase(%q) returned error: %v", driverType, err)
				}
				database, ok := databaseInstance.(*MySQLDB)
				if !ok {
					t.Fatalf("NewDatabase(%q) returned %T, want *MySQLDB", driverType, databaseInstance)
				}
				database.conn = databaseConnection

				err = database.ApplyChangesContext(context.Background(), testCase.tableName, connection.ChangeSet{
				...
				assertMySQLApplyChangesArgs(t, execArgs, [][]interface{}{
					{int64(1)},
					{"updated", int64(2)},
					{"created"},
				})
...
func assertMySQLApplyChangesArgs(t *testing.T, got [][]driver.NamedValue, want [][]interface{}) {
	t.Helper()
	if len(got) != len(want) {
