...
func TestNewGrokCLICommandHidesWindowsConsole(t *testing.T) {
	const existingCreationFlag uint32 = 0x00000004
	const windowsCreateNoWindow uint32 = 0x08000000
	originalCommandContext := grokCommandContext
	t.Cleanup(func() { grokCommandContext = originalCommandContext })
	grokCommandContext = func(ctx context.Context, name string, args ...string) *exec.Cmd {
		cmd := exec.CommandContext(ctx, name, args...)
		cmd.SysProcAttr = &syscall.SysProcAttr{CreationFlags: existingCreationFlag}
		return cmd
	}

	cmd := newGrokCLICommand(context.Background(), "grok.cmd", "--version")
	if cmd.SysProcAttr == nil {
		t.Fatal("expected Windows process attributes to be configured")
	}
	if !cmd.SysProcAttr.HideWindow {
		t.Fatal("expected Grok CLI window to be hidden")
	}
