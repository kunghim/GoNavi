const zhCN = {
  ...
  'workbench.title_compare_short': '数据对比',
  'workbench.title_schema_compare': '表结构比对',
  'workbench.title_data_compare': '数据比对',
  'workbench.subtitle': '定义、验证并追踪跨数据源任务',
  'workbench.subtitle_compare': '只读比较两端结构或数据，不会写入目标',
  'workbench.subtitle_schema_compare': '只读比较两端表结构，不会写入目标',
  'workbench.subtitle_data_compare': '只读比较两端数据，不会写入目标',
  'workbench.start_compare': '开始比对',
  ...
  'task_kind.subtitle_compare': '选择要比对结构还是数据。比对任务不会写入目标。',
  ...
  'task_kind.compare_schema': '表结构比对',
  'task_kind.compare_schema_desc': '只读比较两端表结构，不会写入目标。',
  'task_kind.compare_data': '数据比对',
  'task_kind.compare_data_desc': '只读比较两端数据，不会写入目标。',
  'task_kind.cdc': '持续增量同步',
  'task_kind.cdc_desc': '源库发生增删改时自动同步到目标，并记住已同步到哪。',
  'stage.endpoints': '选择源和目标',
  'stage.mappings': '选择同步数据',
  'stage.mappings_schema_compare': '选择比对对象',
  'stage.mappings_data_compare': '选择比对对象',
  'stage.delivery': '目标写入',
  'stage.trigger': '运行方式',
  'stage.preflight': '检查并运行',
  'stage.preflight_compare': '检查并比对',
  ...
  'editor.guide.source_title': '第一步：选择源',
  'editor.guide.target_title': '第二步：选择目标',
  'editor.guide.target_locked': '先选好源连接，再配置目标。',
  ...
  'metadata.loading_cdc_adapters': '正在识别源库的增量同步方式',
  ...
  'mapping.title_schema_compare': '选择要比对结构的表',
  'mapping.title_data_compare': '选择要比对数据的表',
  'mapping.help': '可选择一张、多张或筛选后的全部表。系统会自动匹配同名目标、识别主键并生成映射，你只需处理例外。',
  'mapping.help_schema_compare': '勾选要比对结构的表。系统按同名匹配目标，只比较结构，不会写入。',
  'mapping.help_data_compare': '勾选要比对数据的表。系统按同名匹配目标，只比较数据，不会写入。',
  'mapping.query_help': 'SQL 查询结果只写入一个目标对象；请选择已有目标表并配置写入键。',
  'mapping.add': '添加映射',
  'mapping.select_objects': '选择源对象',
  'mapping.add_source_objects': '添加源对象',
  'mapping.catalog_title': '可选表',
  'mapping.selected_source': '已选表',
  'mapping.write_to': '写入到',
  'mapping.write_to_compare': '比对对象',
  'mapping.none_selected_title': '还没有选择对象',
  'mapping.none_selected_desc': '在左侧勾选要同步的表，系统会自动匹配同名目标。',
  'mapping.none_selected_desc_compare': '在左侧勾选要比对的表，系统会自动匹配同名目标。',
  ...
  'mapping.exceptions_caption': '{source} 的例外',
  ...
  'mapping.target_hint.exists': '目标库里已经有这张表',
  'mapping.target_hint.create': '目标库里还没有这张表，同步时会自动建出来',
  'mapping.target_hint.missing': '目标库里没有这张表，而当前任务不允许新建',
  'mapping.target_hint.pending': '正在查看目标库里有没有这张表',
  'mapping.key_placeholder': '留空 = 自动按源表主键匹配',
  'mapping.key_detected': '已从源端元数据自动识别，可按需修改',
  'mapping.key_when_needed': '写入已有目标的新增或更新必须有源端物理主键；首次自动建表无需主键。持续增量同步仍需配置稳定键。',
  'mapping.source_placeholder': '搜索或选择源对象',
  'mapping.target_placeholder': '搜索已有目标或输入新名称',
  'mapping.query_result_source': '查询结果（唯一映射）',
  'mapping.target_state.exists': '目标库里已经有这张表',
  'mapping.target_state.create': '目标库里还没有这张表，同步时会自动建出来',
  'mapping.target_state.missing': '目标库里没有这张表，而当前任务不允许新建',
  'mapping.target_state.pending': '正在查看目标库里有没有这张表',
  ...
  'delivery.read_only_note': '任务只读取两端数据，不会写入或删除目标端数据，因此没有写入模式、失败行处理和调度设置。需要补齐结构差异时，请用比较结果里的“生成修复 SQL”，或在数据同步工作台新建结构同步任务。',
  ...
  'delivery.delete.cdc_desc': '源端删除一行后，会按键删除目标端对应行。',
  ...
  'delivery.create_indexes_desc': '自动建表时复制源表的二级索引、归属序列、触发器和字段注释。独立序列、函数、视图不会随表迁移。',
  'trigger.title': '设置运行方式',
  'trigger.help': '选择手动、定时或持续运行，并按需要配置快照、水位线或变更日志增量。',
  'trigger.mode': '触发方式',
  'trigger.manual': '手动',
  'trigger.once': '指定时间',
  ...
  'incremental.cdc_adapter': '增量方式',
  'incremental.select_cdc_adapter': '由源库类型自动选择',
  'incremental.cdc_probe_ready': '已确认当前连接、权限和部署方式支持自动选择的增量同步。',
  'incremental.cdc_probe_unready': '自动选择的增量同步方式当前还不能用。',
  'incremental.start_position': '开始位置',
  'incremental.position.latest': '最新位置',
  'incremental.position.earliest': '最早位置',
  'incremental.position.checkpoint': '上次进度',
  'incremental.cdc_safety_title': '增量起点说明',
  'incremental.cdc_safety_note': '目前只能从“现在之后的新变更”开始，或从本任务上次记下的进度继续。同一条变更有可能被处理不止一次。',
  'incremental.cdc_snapshot_gap_warning': '如果先单独做一次全量，再从最新位置启动，中间这段时间的变更可能漏掉。',
  'incremental.cdc_checkpoint_unavailable': '当前任务还没有可继续的同步进度，因此不能从上次进度开始。',
  'preflight.title': '预检',
  ...
  'runs.title': '运行记录',
  'runs.subtitle': '查看每次运行的状态和结果。进行中的阶段和事件会自动更新。',
  'runs.subtitle_compare': '查看每次比对的状态和结果。进行中的阶段和事件会自动更新。',
  'runs.empty_title': '还没有运行记录',
  'runs.empty_desc': '保存并运行任务后，这里会显示写入量、失败情况和同步进度。',
  'runs.empty_desc_compare': '保存并开始比对后，这里会显示比对结果。',
  'runs.id': '运行 ID',
  'runs.task': '任务',
  'runs.status': '状态',
  ...
  'runs.next_page': '下一页',
  'runs.delete_confirm': '删除此运行记录及其错误行、事件明细？此操作不会删除同步进度。',
  'runs.clear_terminal_confirm': '清除全部已结束运行记录及其错误行、事件明细？此操作不会删除同步进度。',
  'runs.delete_confirm_title': '删除运行记录',
  'runs.clear_terminal_confirm_title': '清除已结束记录',
  'events.title': '事件时间线',
  'events.title_secondary': '运行过程',
  ...
  'compare.actions.repair_sql': '生成修复 SQL',
  'compare.actions.ai_sql': 'AI 分析差异',
  'compare.actions.sync_diff': '同步差异',
  'compare.repair_sql_tab': '比对修复 SQL',
  'compare.sync_task_name': '{name} · 差异同步',
  'checkpoint.title': '同步进度',
  'checkpoint.empty': '所选运行没有保存同步进度。',
  'checkpoint.kind': '类型',
  'checkpoint.phase': '阶段',
  'checkpoint.updated_at': '更新时间',
  'checkpoint.cursor': '进度位置',
  'checkpoint.reset': '重置同步进度',
  'checkpoint.reset_confirm_title': '重置同步进度',
  'checkpoint.reset_warning': '重置后不能从该位置继续；持续增量同步只能从重置之后的新变更开始。',
  'common.yes': '是',
  'common.no': '否',
  'common.refresh': '刷新',
  'common.dismiss': '关闭',
  ...
  'cdc.empty_desc': '创建持续增量同步任务并完成权限检查后，这里会显示健康状态。',
  'cdc.lag_unknown': '延迟未上报',
  'cdc.status.ready': '已就绪',
...
const enUS: Record<DataSyncWorkbenchTextKey, string> = {
  ...
  'workbench.title_compare_short': 'Data compare',
  'workbench.title_schema_compare': 'Schema compare',
  'workbench.title_data_compare': 'Data compare',
  'workbench.subtitle': 'Define, validate, and track cross-source jobs',
  'workbench.subtitle_compare': 'Read-only comparison of schema or data; the target is never written',
  'workbench.subtitle_schema_compare': 'Read-only schema comparison; the target is never written',
  'workbench.subtitle_data_compare': 'Read-only data comparison; the target is never written',
  'workbench.start_compare': 'Start compare',
  ...
  'task_kind.subtitle_compare': 'Choose schema or data comparison. Compare tasks never write to the target.',
  ...
  'task_kind.compare_schema': 'Schema compare',
  'task_kind.compare_schema_desc': 'Read-only schema comparison; the target is never written.',
  'task_kind.compare_data': 'Data compare',
  'task_kind.compare_data_desc': 'Read-only data comparison; the target is never written.',
  ...
  'stage.mappings_schema_compare': 'Choose objects',
  'stage.mappings_data_compare': 'Choose objects',
  'stage.delivery': 'Target writes',
  'stage.trigger': 'Run settings',
  'stage.preflight': 'Check and run',
  'stage.preflight_compare': 'Check and compare',
  ...
  'route.summary_compare': 'Compare path',
  'route.source': 'Source',
  'route.target': 'Target',
  ...
  'editor.guide.source_title': 'Step 1: Choose the source',
  'editor.guide.target_title': 'Step 2: Choose the target',
  'editor.guide.target_locked': 'Select a source connection before configuring the target.',
  ...
  'mapping.title_schema_compare': 'Choose tables to compare schema',
  'mapping.title_data_compare': 'Choose tables to compare data',
  ...
  'mapping.help_schema_compare': 'Check tables whose schema should be compared. Same-name targets are matched. Nothing is written.',
  'mapping.help_data_compare': 'Check tables whose data should be compared. Same-name targets are matched. Nothing is written.',
  ...
  'mapping.catalog_title': 'Available tables',
  'mapping.selected_source': 'Selected table',
  'mapping.write_to': 'Write to',
  'mapping.write_to_compare': 'Compare with',
  'mapping.none_selected_title': 'No objects selected',
  'mapping.none_selected_desc': 'Check tables on the left. Matching targets are filled in automatically.',
  'mapping.none_selected_desc_compare': 'Check tables on the left to compare. Matching targets are filled in automatically.',
  ...
  'mapping.select_all': 'Select all ({count})',
  ...
  'mapping.exceptions_caption': 'Exceptions for {source}',
  ...
  'mapping.target_hint.exists': 'This table already exists on the target',
  'mapping.target_hint.create': 'This table is not on the target yet; it will be created during sync',
  'mapping.target_hint.missing': 'This table is not on the target, and this task is not allowed to create it',
  'mapping.target_hint.pending': 'Checking whether this table already exists on the target',
  ...
  'mapping.target_state.exists': 'This table already exists on the target',
  'mapping.target_state.create': 'This table is not on the target yet; it will be created during sync',
  'mapping.target_state.missing': 'This table is not on the target, and this task is not allowed to create it',
  'mapping.target_state.pending': 'Checking whether this table already exists on the target',
  ...
  'delivery.read_only_note': 'The task only reads both sides; it never writes to or deletes target data, so there is no write mode, failed-row handling, or schedule. To close structure gaps, use “Generate repair SQL” in the compare result, or create a structure sync task in the data sync workbench.',
  ...
  'delivery.create_indexes_desc': 'When auto-creating the target table, copy secondary indexes, owned sequences, triggers, and column comments. Standalone sequences, functions, and views are not migrated with the table.',
  ...
  'runs.subtitle': 'Review each run’s status and results. Active stages and events update automatically.',
  'runs.subtitle_compare': 'Review each comparison’s status and results. Active stages and events update automatically.',
  'runs.empty_title': 'No runs yet',
  'runs.empty_desc': 'After a task runs, writes, failures, and sync progress appear here.',
  'runs.empty_desc_compare': 'After a comparison runs, the results appear here.',
  'runs.id': 'Run ID',
  'runs.task': 'Task',
  'runs.status': 'Status',
  ...
  'runs.checkpoint': 'Sync progress',
  'runs.view_errors': 'View error rows',
  'runs.view_details': 'View run details',
  'runs.back_to_list': 'Back to run history',
  ...
  'runs.delete_confirm': 'Delete this run record and its error rows and event details? Sync progress is kept.',
  'runs.clear_terminal_confirm': 'Clear all completed run records and their error rows and event details? Sync progress is kept.',
  ...
  'events.title_secondary': 'Run process',
  ...
  'compare.actions.repair_sql': 'Generate repair SQL',
  'compare.actions.ai_sql': 'Analyze diffs with AI',
  'compare.actions.sync_diff': 'Sync differences',
  'compare.repair_sql_tab': 'Compare repair SQL',
  'compare.sync_task_name': '{name} · Diff sync',
  'checkpoint.title': 'Sync progress',
  'checkpoint.empty': 'The selected run has no saved sync progress.',
  'checkpoint.kind': 'Kind',
  'checkpoint.phase': 'Phase',
  'checkpoint.updated_at': 'Updated',
  'checkpoint.cursor': 'Position',
  'checkpoint.reset': 'Reset sync progress',
  'checkpoint.reset_confirm_title': 'Reset sync progress',
  'checkpoint.reset_warning': 'Reset removes the resume position. Continuous incremental sync can then start only from new changes after the reset.',
  'common.yes': 'Yes',
  'common.no': 'No',
