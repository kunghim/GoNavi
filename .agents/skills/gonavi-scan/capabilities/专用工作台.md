...
- 已知限制：Nacos 使用专用服务链路，但通用查询能力门控仍会把它暴露到部分 SQL 工作流。
...
- 已知限制：JVM 使用专用 Provider 链路，但通用查询能力门控仍会把它暴露到部分 SQL 工作流。
...
- 已知限制：单机模式支持 SSH；Cluster/Sentinel 与 SSH 组合后端拒绝，连接弹窗尚未提前门控。
...
| CAP-REDIS-A07 | 导入导出 Redis Key | Redis 工具 → 预览导入/选择导出 → 执行 | 未支持类型显式报错；高可用拓扑 SSH 组合限制见 CAP-REDIS-001 | 部分实现 | `RedisExportKeys`；`RedisImportKeys`；`migration_redis.go` |
...
| CAP-MSG-A02 | 从连接树进入消息对象工作流 | 连接树 → Topic/Queue/Exchange/Topic Filter → 打开查询或发布 | 本项仅作聚合入口索引，具体能力见消息数据源分册 | 已实现 | 四类消息驱动与侧栏对象菜单 |
...
| ID | 具体能力 | 用户入口与操作 | 边界/异常 | 状态 | 证据 |
|---|---|---|---|---|---|
| CAP-NACOS-A01 | 连接和浏览命名空间 | 新建 Nacos 连接 → 认证 → 命名空间树 | API v1/v2、认证刷新和缓存；SQL 入口门控尚未完全隔离 | 部分实现 | `methods_nacos.go`；`internal/nacos/client.go`；`dataSourceCapabilities.ts` |
...
| ID | 具体能力 | 用户入口与操作 | 边界/异常 | 状态 | 证据 |
|---|---|---|---|---|---|
| CAP-JVM-A01 | JMX、HTTP 或 Agent 连接 | 新建 JVM 连接 → Provider → 测试 | fixture 已统一 JDK 工具链；SQL 入口门控尚未完全隔离 | 部分实现 | `methods_jvm.go`；`fixture_toolchain_test.go`；`http_provider_test.go`；`jmx_provider_test.go` |
