...
| CAP-DS-SQLSERVER | SQL Server，可选 Driver Agent | 库表、SQL、列/索引/外键 | DDL 未实现时返回明确 unsupported，不再伪成功 | 部分实现 | `sqlserver_impl.go`；`sqlserver_impl_test.go` |
...
| CAP-DS-MONGODB | MongoDB，可选 Driver Agent | 集合、文档查询、URI 和成员发现 | SRV 与普通 URI 均可通过成员级 SSH dialer；真实 SRV 跳板环境待验证 | 部分实现 | `mongodb_impl.go`；`mongodb_impl_v1.go`；`MongoDiscoverMembers` |
