...
| CAP-CONN-A08 | SSH 隧道 | 网络安全 → SSH 主机/密钥 → 测试 | RocketMQ 和 MongoDB SRV 已支持动态成员地址转发；其他数据源仍按驱动能力处理 | 部分实现 | `ConnectionModalNetworkSecuritySection.tsx`；`internal/ssh`；`rocketmq_tunnel.go`；`mongodb_impl.go` |
| CAP-CONN-A09 | TLS/SSL 证书 | 网络安全 → TLS 模式/证书 → 测试 | 各驱动 SSL mode 映射不同 | 已实现 | `internal/tlsconfig`；`ssl_mode.go` |
| CAP-CONN-A10 | 全局与连接代理 | 设置/连接 → 代理 → 测试 → 保存 | RocketMQ 已支持代理与 HTTP 隧道；MQTT WebSocket 组合仍被拒绝 | 部分实现 | `global_proxy.go`；`internal/proxy`；`rocketmq_tunnel.go`；`mqtt_impl.go` |
